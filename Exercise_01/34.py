...
def pipeline_model(img_path):
    ...
    faceDetectionModel = os.path.join(path, 'res10_300x300_ssd_iter_140000_fp16.caffemodel')
 
    # Face detection model architecture
    faceDetectionProto = os.path.join(path, 'deploy.prototxt.txt.prototxt')
 
    # Face descriptor model
    faceDescriptor = os.path.join(path, 'openface.nn4.small2.v1.t7')
...
image_path = os.path.join(path, 'FaceImage/1721031236_nguyenhoangtuan/image_120.jpg')
